...
var strictComposite = []; //The array of StrictComposite
var extendedComposite = []; //The array of ExtendedComposite
                ...
                if(err){
                    ...
                    if(!num){
                        ...
                        for(var i = 0; i < Class.length; i++){
                            ...
                            // remove doublicates
                            Class[i].key = Class[i].key.filter(function(item, pos, self) {
                            ...
                            if(path === undefined){
                                if(Class[i].key.array){
                                    ...
                                    if(Class[i].key !== ""  && Class[i].key.length !== 0 ){
                            ...
                            if(Class[i].type === "DataType" && Class[i].nodeType === "grouping" && Class[i].generalization.length === 0){
                                if(Class[i].attribute.length === 1){
                                    if(!Class[i].attribute[0].isUses){
                                        ...
                                        Class[i].attribute = [];
                                        Class[i].name = Util.yangifyName(Class[i].name);
                        ...
                        var identityAlreadyInFile = [];
                        for(var i = 0; i < Identity.length; i++){
//                            Identity[i].name+="-id";
                            for(var  j = 0; j < packages.length; j++){
                                if(Identity[i].fileName == packages[j].fileName && packages[j].name.toLowerCase()=="typedefinitions"){
                                    var exists = identityAlreadyInFile.filter(function(item){
                                        return item === packages[j].fileName + Identity[i].name;
                                    });
                                    if (exists.length === 0) {
                                        packages[j].children.push(Identity[i]);
                                        identityAlreadyInFile.push(packages[j].fileName + Identity[i].name);
                                    }
                                }
                            }
                        }
...
function parseStrictCom(xmi){
    var id;
    if(xmi.attributes()["base_Association"]){
        id = xmi.attributes()["base_Association"];
    }
     strictComposite.push(id);

}

function parseExtendedCom(xmi){
    var id;
    if(xmi.attributes()["base_Association"]){
        id = xmi.attributes()["base_Association"];
    }
    extendedComposite.push(id);
}
               ...
               /* var specTargetFlag = false;
                ...
                if (r !== "basicType") {
                    ...
                    /*for(var k=0;k<association.length;k++) {
                        if (node.attribute[i].association == association[k].assoid) {
                            if (association[k].strictCom == true) {
                                node.attribute[i].isleafRef = false;
                                if (association[k].upperValue !== 1) {
                                    node.attribute[i].nodeType == "list";
                                } else {
                                    node.attribute[i].nodeType == "container";
                                }
                            } else if (association[k].extendedCom == true) {
                                node.attribute[i].isleafRef = false;
                                node.generalization.push(node.attribute[i].association);
                                //node.attribute[i]=null;
                            } else {
                                node.attribute[i].isleafRef = true;
                                if (association[k].upperValue !== 1) {
                                    node.attribute[i].nodeType == "leaf-list";
                                } else {
                                    node.attribute[i].nodeType == "leaf";
                                }
                            }
                        }
                    }*/
                    /*for(var m=0; m<strictComposite.length; m++){
                        if(strictComposite[m] == node.attribute[i].association){
                            node.attribute[i].isleafRef=false;
                            node.attribute[i].nodeType == "list"
                            break;
                        }
                    }
                    for(var n=0; n<extendedComposite.length; n++){
                        if(extendedComposite[n] == node.attribute[i].association){
                            //delete this attribute,and add uses to this class
                            node.generalization.push(node.attribute[i].association);
                            //node.attribute[i]=null;
                            break;
                        }
                    ...
                    if(!node.attribute[i].isleafRef && node.type == "Class"){
                        ...
                        if(r == node.id){
                            ...
                            console.warn("Warning:xmi:id= " + node.name + " (" + r + ") is compositeed by itself!");
                        }
...
//                node.name+="-t";
            ...
            if(node.isLeaf == true){
                ...
                node.buildEnum(obj);
                node.name = Util.yangifyName(node.name);
                Typedef.push(node);
            } else{
                ...
                Typedef.push(node);
                    ...
                    name.replace(/-t$/g,"");
                    var nodeI = new Node(Util.yangifyName(name),"","identity");
                    nodeI.fileName=node.fileName;
                    var exists = Identity.filter(function(item){
                        return item.name === nodeI.name;
                    });
                    if (exists.length === 0) {
                        Identity.push(nodeI);
                    }
                    ...
                    if (literal.array != undefined) {
                        for (var i = 0; i < literal.array.length; i++) {
                            enumValue = nodeI.name + '-' + Util.yangifyName(literal.array[i].attributes().name);
                            enumComment = "";
...
function createAssociation(obj) {
    var ele;
    var len;
    var assoid = obj.attributes()["xmi:id"];
    var strictCom = false;
    var extendedCom = false;
    var comflag=false;
    //if(ob)
    for(var i=0; i<strictComposite.length; i++){
        if(strictComposite[i] == assoid){
            strictCom =true;
            comflag=true;
            break;
        }
    }
    for(var j=0; j<extendedComposite.length; j++){
        if(extendedComposite[j] == assoid){
            extendedCom=true;
            comflag=true;
            break;
        }
    ...
    if (obj.ownedEnd) {
        ...
        for (var i = 0; i < len; i++) {
            ...
            var upperValue;
            ele.upperValue ? upperValue = ele.upperValue.attributes().value : "";
            var lowerValue;
            ele.lowerValue ? lowerValue = ele.lowerValue.attributes().value : "";
            //if (parseInt(upperValue) !== 1) {
                for(var j = 0; j < association.length; j++){
                    if(id == association[j].id){
                        break;
                    }
                }
                if(j == association.length){
                    var a = new assoc(name, id, "list", upperValue, lowerValue,assoid,strictCom,extendedCom);
                    association.push(a);
                    //console.log(association.length);
                }
            //}
        }
    }
    ...
    for(var i = 0; i < ele.length; i++){
        ...
        if(ele[i].nodeType == "grouping" || ele[i].nodeType == "notification"){
            ...
            for (var j = 0; j < ele[i].attribute.length; j++) {
                ...
                for(var k=0;k<association.length;k++) {
                    if (ele[i].attribute[j].association && ele[i].attribute[j].association == association[k].assoid) {
                        if (association[k].strictCom == true) {
                            ele[i].attribute[j].isleafRef = false;
                            if (association[k].upperValue >1||association[k].upperValue=="*") {
                            ...
                            ele[i].attribute[j].isleafRef = false;
                            for(var l = 0; l < Class.length; l++){
                                if(Class[l].id == ele[i].attribute[j].type){
                                    var name;
                                    if(Class[l].fileName!=currentFileName){
                                        name=Class[l].fileName.split('.')[0] + ":" + Class[l].name;
                                    }else{
                                        name=Class[l].name;
                                    }
                ...
                /*for(var m=0; m<strictComposite.length; m++){
                    if(strictComposite[m] == ele[i].attribute[j].association){
                        ele[i].attribute[j].isleafRef=false;
                        break;
                    }
                }
                for(var n=0; n<extendedComposite.length; n++){
                    if(extendedComposite[n] == ele[i].attribute[j].association){
                        ele[i].attribute[j].isleafRef=false;
                        break;
                    }
                }*/
                ...
                if(ele[i].attribute[j].type.range !== undefined){
                    ...
                    if(regex.test(ele[i].attribute[j].type.range) === true){
                        ...
                        console.warn("Warning: The range of id = \"" + ele[i].attribute[j].name + "\" (" + ele[i].attribute[j].type.id + ") doesn't match the RFC 6020! We will put this range into description. Please recheck it.");
                    }else{
                        if(ele[i].attribute[j].type.range.indexOf('*') !== -1){
                            ele[i].attribute[j].type.range = this.range.replace('*', "max");
                        }
                ...
                if(pValue.isUses){
                    ...
                    for(var k = 0; k < Class.length; k++){
                        if(Class[k].id === name){
                            ...
                            //recursion
                            if(i === k){
                                console.info('xxx',tClass[k].instancePathhis.type);
