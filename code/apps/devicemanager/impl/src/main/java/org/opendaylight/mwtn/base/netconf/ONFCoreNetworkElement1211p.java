...
import org.opendaylight.mwtn.devicemanager.impl.listener.MicrowaveEventListener1211p;
...
public class ONFCoreNetworkElement1211p extends ONFCoreNetworkElementBase {
    ...
    private final @Nonnull MicrowaveEventListener1211p microwaveEventListener;
    ...
    private final boolean isNetworkElementCurrentProblemsSupporting1211p;
    private ListenerRegistration<MicrowaveEventListener1211p> listenerRegistrationresult;
     ...
     */
    private ONFCoreNetworkElement1211p(String mountPointNodeName, Capabilities capabilities,
        ...
        this.microwaveEventListener = new MicrowaveEventListener1211p(mountPointNodeName, webSocketService,
                databaseService, dcaeProvider, aotsmClient, maintenanceService2, notificationDelayService);
        this.isNetworkElementCurrentProblemsSupporting1211p = capabilities
                .isSupportingNamespaceAndRevision(NetworkElementPac.QNAME);
        LOG.debug("support necurrent-problem-list=" + this.isNetworkElementCurrentProblemsSupporting1211p);
     ...
     */
    public static @Nullable ONFCoreNetworkElement1211p build(String mountPointNodeName, Capabilities capabilities,
            DataBroker netconfNodeDataBroker, WebSocketServiceClient webSocketService,
            HtDatabaseEventsService databaseService, ProviderClient dcaeProvider, @Nullable ProviderClient aotsmClient,
            MaintenanceService maintenanceService2, NotificationDelayService notificationDelayService) {
        return checkType(capabilities)
                ? new ONFCoreNetworkElement1211p(mountPointNodeName, capabilities, netconfNodeDataBroker,
...
//        }
        ...
        if (isNetworkElementCurrentProblemsSupporting1211p) {
            idxStart = resultList.size();
            readNetworkElementCurrentProblems1211p(resultList);
            debugResultList("CurrentProblems1211p", resultList, idxStart);
        }
            ...
            if (airHistoricalPerformanceData == null) {
                ...
                if (airHistPMList != null) {
                    for (AirInterfaceHistoricalPerformanceTypeG pmRecord : airHistoricalPerformanceData
                            .getHistoricalPerformanceDataList()) {
                        resultList.add(
                                new ExtendedAirInterfaceHistoricalPerformanceType1211p(pmRecord, airConfiguration));
                    }
                }
            }
        }
...
//    }
     ...
     */
    private List<ProblemNotificationXml> readNetworkElementCurrentProblems1211p(
            List<ProblemNotificationXml> resultList) {

        LOG.info("DBRead Get {} NetworkElementCurrentProblems1211p", mountPointNodeName);
        ...
        try {
            ...
            if (problems == null) {
                LOG.debug("DBRead no NetworkElementCurrentProblems1211p");
            } else if (problems.getCurrentProblemList() == null) {
                LOG.debug("DBRead empty CurrentProblemList1211p");
            } else {
            ...
            LOG.warn("DBRead {} NetworkElementCurrentProblems1211p not supported. Message '{}' ", mountPointNodeName,
                    e.getMessage());
        }
        return resultList;
