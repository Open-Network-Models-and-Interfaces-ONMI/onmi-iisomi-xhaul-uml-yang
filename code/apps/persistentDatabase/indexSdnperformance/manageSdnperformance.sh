# manageSdnperformance script
# (c)2017 higstreet technologies
Version=V1.0


URL="http://localhost:9200"
BASENAME=sdnperformance
BASESEP="_"
BASEVER="v"
...
createIndex() {
   echo Create index $1
   curl -H "Content-Type: application/json" --data @sdnperformanceMapping.json $URL/$1
   echo
}

createAlias() {
   echo Create alias for index
   curl -H "Content-Type: application/json" --data @sdnperformanceV1Alias.json $URL/_aliases
   echo
}
...
checkElasticdump() {
    elasticdump --version >/dev/null 2>&1 || { echo ; echo "Error: I require elasticdump but it's not installed.  Aborting."; exit 1; }
}
...
echo "------------------------------------"
echo "sdnperformance management script $Version"
