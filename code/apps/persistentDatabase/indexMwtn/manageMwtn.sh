...
createIndex() {
   echo Create index $1
   curl -H "Content-Type: application/json" --data @mwtnMapping.json $URL/$1
   echo
}

createAlias() {
   echo Create alias for index
curl -H "Content-Type: application/json" --data @mwtnV1Alias.json $URL/_aliases
   echo
}
...
curl -H "Content-Type: application/json" --data @mwtnSwapAlias.json $URL/_aliases
