...
var summaryFile = __dirname+test_cases["SummaryReportFile02"];
...
function executeGetSet(url, leafData, chainType) {
    ...
    for (var i = 0; i < leafData.length; i++) {
        ...
        if(isValidJson(checkJson) && leafData[i].leafCurrentValue != null && leafData[i].leafCurrentValue != "" && comma_flag == false){
			appendSync("{\"Leaf Name\": \"" + leafData[i].leafName + "\", \"Parent Node\": \"" + leafData[i].parentNode + "\", \"Current Value\": \"" + leafData[i].leafCurrentValue + "\", \"Status\": \"Retrieved\"}", summaryFileHandle);
            comma_flag = true;
