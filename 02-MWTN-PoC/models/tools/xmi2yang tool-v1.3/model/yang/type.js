...
type.prototype.writeNode = function (layer) {
   ...
   /* if (this.name !== "enumeration") {
    ...
    // TODO sko hack
    if (this.path) {
      if (this.path.contains('/CoreModel-CoreNetworkModule-ObjectClasses:NetworkElement/_ltpRefList/_lpList/uuid')) {
        // console.info('INFO', this.path, 'changed to string');
        s = PRE + 'type string;' + '\r\n';
        s += PRE + '// type leafref {' + '\r\n';
        s += PRE + '//    path "CoreModel-CoreNetworkModule-ObjectClasses:NetworkElement/_ltpRefList/_lpList/uuid";' + '\r\n';
        s += PRE + '// }' + '\r\n';
      }
    }
    return s;
