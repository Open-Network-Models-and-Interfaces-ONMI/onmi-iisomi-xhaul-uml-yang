...
function Node(name, descrip, type, maxEle, minEle, id, config,isOrdered,feature,status) {
  if (name.contains('MW_')) {
    console.info('CHECK:', 'type', type, 'changed to current for ', name);
...
Node.prototype.writeNode = function (layer) {
    ...
    // TODO [sko] if (this.nodeType == "container" && this.config || this.nodeType == "list" && this.config) {
    if (this.nodeType == "container" || this.nodeType == "list" ) {
        conf = PRE + "\tconfig " + this.config + ";\r\n";
    } else {
        conf = "";
    }
    // TODO current workaround: always expose config for container:
