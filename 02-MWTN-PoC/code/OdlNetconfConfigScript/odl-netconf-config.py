...
for configFile in os.listdir(configDir):
    ...
    try:
        ...
        if operation == "add":
            ...
            url = "http://{}:{}/restconf/config/network-topology:network-topology/topology/topology-netconf/node/controller-config/yang-ext:mount/config:modules/module/odl-sal-netconf-connector-cfg:sal-netconf-connector/{}".format(
                odlIP, odlPort, name)
            headers = {
                'content-type': "application/xml",
                'cache-control': "no-cache"
